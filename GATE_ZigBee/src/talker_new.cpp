...
/** 
 * Publication de capIDS pour le service de découverte
 */

void publish_discover(ros::NodeHandle node, std_msgs::String msg){

   pubsensID = node.advertise<std_msgs::String>("SensorIDs", 1000);
   ROS_INFO("capID %s", msg.data.c_str());
   pubsensID.publish(msg);
      ros::spinOnce();
}
...
/**Get Hostname
  ...
  while (1) //ros::ok()
  {
   ...
   // ROS_INFO("%s", msg.frame_id.c_str());
	...
	// Service de découverte capID
	std_msgs::String msgID;
	msgID.data = capID.str(); 
